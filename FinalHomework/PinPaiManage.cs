...
{
    public partial class PinPaiManage : Form
    {
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == dataGridView1.Columns["修改"].Index && e.RowIndex >= 0)
            {
                ...
                {
                    connection.Open();
                    string sql = "UPDATE pinpai SET Mc = @Mc where Dm=@Dm";
            ...
            if (e.ColumnIndex == dataGridView1.Columns["删除"].Index && e.RowIndex >= 0)
            {
                ...
                {
                    connection.Open();
                    string sql = "DELETE FROM pinpai WHERE Dm = @Dm";
                    SqlCommand cmd = new SqlCommand(sql, connection);
                    cmd.Parameters.AddWithValue("@Dm", Dm);
                    cmd.ExecuteNonQuery();
                }
        ...
        private void PinPaiManage_Load(object sender, EventArgs e)
        {
            string query = "SELECT * FROM pinpai";
            // 创建数据库连接对象
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                // 打开数据库连接
        ...
        {
            AddPinpaiName addPinpaiName = new AddPinpaiName();
            addPinpaiName.ShowDialog();
        }
        ...
        private void 查询_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    dataGridView1.Rows.Clear();
                    query = "SELECT * FROM pinpai";

                }//只有代码
                else if ( string.IsNullOrEmpty(Mc))
                {
                    query = "SELECT * FROM pinpai where Dm=@Dm";
                    cmd = new SqlCommand(query, connection);
                    cmd.Parameters.AddWithValue("@Dm", Dm);
                }//只有名称条件
                else if(string.IsNullOrEmpty(Dm))
                {
                    query = "SELECT * FROM pinpai where Mc=@Mc";
                    cmd = new SqlCommand(query, connection);
                    cmd.Parameters.AddWithValue("@Mc", Mc);
                }//全有
                else
                {
                    query = "SELECT * FROM pinpai where Mc=@Mc and Dm = @Dm";
                    cmd = new SqlCommand(query, connection);
                    cmd.Parameters.AddWithValue("@Mc", Mc);
                    cmd.Parameters.AddWithValue("@Dm", Dm);
                }
