...
{
    public partial class xiaolei : Form
    {
        ...
        private void xiaolei_Load(object sender, EventArgs e)
        {
            string query = "SELECT * FROM xiaolei";
            // 创建数据库连接对象
        ...
        {
            AddxiaoleiName addxiaoleiName = new AddxiaoleiName();
            addxiaoleiName.ShowDialog();
        }
        ...
        private void 查询_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    dataGridView1.Rows.Clear();
                    query = "SELECT * FROM xiaolei";

                }//只有代码
                else if (string.IsNullOrEmpty(Mc))
                {
                    query = "SELECT * FROM xiaolei where Dm=@Dm";
                    cmd = new SqlCommand(query, connection);
                    cmd.Parameters.AddWithValue("@Dm", Dm);
                }//只有名称条件
                else if (string.IsNullOrEmpty(Dm))
                {
                    query = "SELECT * FROM xiaolei where Mc=@Mc";
                    cmd = new SqlCommand(query, connection);
                    cmd.Parameters.AddWithValue("@Mc", Mc);
                }//全有
                else
                {
                    query = "SELECT * FROM xiaolei where Mc=@Mc and Dm = @Dm";
                    cmd = new SqlCommand(query, connection);
                    cmd.Parameters.AddWithValue("@Mc", Mc);
                    cmd.Parameters.AddWithValue("@Dm", Dm);
                }
        ...
        private void 新增_Click_1(object sender, EventArgs e)
        {
            AddxiaoleiName addxiaoleiName = new AddxiaoleiName();
            addxiaoleiName.ShowDialog();
        }

        private void 刷新_Click_1(object sender, EventArgs e)
        {
            dataGridView1.Rows.Clear();
            string query = "SELECT * FROM xiaolei";
            // 创建数据库连接对象
        ...
        private void 查询_Click_1(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    query = "SELECT * FROM xiaolei where Dm like @Dm";
                    cmd = new SqlCommand(query, connection);
                    cmd.Parameters.AddWithValue("@Dm", Dm + "%");
                }//只有名称条件
                else if (string.IsNullOrEmpty(Dm))
                ...
                {
                    query = "SELECT * FROM xiaolei where Mc=@Mc and Dm like @Dm";
                    cmd = new SqlCommand(query, connection);
                    cmd.Parameters.AddWithValue("@Mc", Mc);
                    cmd.Parameters.AddWithValue("@Dm", Dm + "%");
                }
        ...
        private void dataGridView1_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == dataGridView1.Columns["修改"].Index && e.RowIndex >= 0)
            {
                ...
                {
                    connection.Open();
                    string sql = "UPDATE xiaolei SET Mc = @Mc where Dm=@Dm";
            ...
            if (e.ColumnIndex == dataGridView1.Columns["删除"].Index && e.RowIndex >= 0)
            {
                ...
                {
                    connection.Open();
                    string sql = "DELETE FROM xiaolei WHERE Dm = @Dm";
                    SqlCommand cmd = new SqlCommand(sql, connection);
                    cmd.Parameters.AddWithValue("@Dm", Dm);
                    cmd.ExecuteNonQuery();
                }
